    ...
    def _fit_spark(self, data):
        ...
        if self.method == "als":
            ...
            while (alsIter < self.maxIter) and (hConvCurr > self.tol):
                ...
                hConvergence.append(hConvCurr)
                if wConvergence is not None:
                    if wOld is not None:
                        wConvergence.append(rddFrobeniusNorm(w, wOld))
                    else:
                        wConvergence.append(inf)

                # calculate reconstruction error
                if self.reconHist == 'all':
                    recData = w.mapValues(lambda x: dot(x, h))
                    reconErr.append(rddFrobeniusNorm(mat, recData))
            ...
            if self.reconHist == 'final':
                    recData = w.mapValues(lambda x: dot(x, h))
                    reconErr = rddFrobeniusNorm(mat, recData)
